...
class WaybillRepositoryImpl @Inject constructor(
    ...
    override suspend fun conductWaybill(params: ConductWaybillParams): Try<Unit, Throwable> =
    ...
    override suspend fun rollbackWaybill(params: RollbackWaybillParams): VoidTry<Throwable> =
        runOperationCatching {
            val dtoToUpdate = waybillMapper.domainToDto(params.waybill)
            waybillApi.updateWaybill(
                UpdateWaybillRequestDTO(dtoToUpdate)
    ...
    override fun getAcceptanceListPaging(): Flow<PagingData<Waybill>> = Pager(
        config = pagingConfig
    ) {
    ...
    override fun getProducts(params: GetWaybillProductsParams): Flow<PagingData<WaybillProduct>> =
        Pager(
            config = pagingConfig
        ) {
    ...
    override suspend fun createDraftWaybill(): Try<Waybill, Throwable> = runOperationCatching {
