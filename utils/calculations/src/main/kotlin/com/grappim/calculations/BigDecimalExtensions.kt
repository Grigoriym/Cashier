...
fun String?.asBigDecimal(scale: Int = 2): BigDecimal = when {
    this.isNullOrBlank() -> {
...
fun String?.asBigDecimalOrNull(scale: Int = 3): BigDecimal? = try {
...
fun BigDecimal.isLessThanOrEquals(value: BigDecimal): Boolean = this.compareTo(value) == -1 ||
